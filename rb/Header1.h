...
public:
	...
	void insert(T key);    // Вставляем узел, внешний интерфейс
...
private:
	...
	void remove(RBTNode<T>*& root, RBTNode<T>* node);// Удалить узел 
...
void RBTree<T>::InsertFixUp(RBTNode<T>*& root, RBTNode<T>* node)
{
	...
	while (node != RBTree::root && parent->color == Red)
	{
		...
		{
			RBTNode<T>* uncleJohn = gparent->right;
			if (uncleJohn != NULL && uncleJohn->color == Red)
			{
				parent->color = Black;
				uncleJohn->color = Black;
				gparent->color = Red;
				node = gparent;
				parent = node->parent;
			}
